...
import 'package:spotify_clone/logic/bloc/internet_cubit.dart';
...
class _DailyMixState extends State<DailyMix> {
  ...
  void initState() {
    ...
    //getAlbum();
    //print("Album trackkk:${kygoAlbum?.tracks} ");
    getkygoAlbum = API().getKygoAlbum("7tcs1X9pzFvcLOPuhCstQJ");
    ...
    scrollController.addListener(() {
      ...
      print("containerHeight: $containerHeight");
      print("${-containerHeight.clamp(100.0, double.infinity).toDouble()}");
      topB == topbar(context);
      setState(() {});
    });
 ...
 */
  ...
  AnimatedOpacity topbar(BuildContext context) {
    return AnimatedOpacity(
      ...
      child: Container(
        decoration: BoxDecoration(color: const Color.fromARGB(255, 66, 66, 80)),
        child: SafeArea(
          child: SizedBox(
            width: MediaQuery.of(context).size.width,
  ...
  Container mixSongs(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          ...List.generate(int.parse(kygoAlbum!.total_tracks), (index) {
            albumfile.Track track = kygoAlbum!.tracks[index];
            return Padding(
              padding: const EdgeInsets.symmetric(vertical: 15),
              child: ListTile(
                leading: Container(
                  child: Image(
                      image: NetworkImage(kygoAlbum!.images[0].url),
                ...
                subtitle: RichText(
                    text: TextSpan(
                        ...
                        children: List.generate(track.artists.length, (index) {
                          return index != track.artists.length
    ...
    return Container(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(
              "${kygoAlbum!.artists[0].name} : ${kygoAlbum!.name} ", //"Anne-Marie, Dra Project, Tom Grennan and more",
              style:
